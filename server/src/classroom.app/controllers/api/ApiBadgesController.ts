...
import { BadgesSrv } from '../../services/BadgesSrv';
 ...
 export class ApiBadgesController {
     ...
     save(@Body({ validate: true, required: true }) entity: BadgesModel) {            
         return this.badgesSrv.save(entity);
     }

     @Put("/:id")
     @UseBefore(AdminsAndTeachersOnly)
     update(@Param("id") id: number, @Body({ validate: true, required: true }) entity: BadgesModel) {            
         entity.id = id;
         return this.badgesSrv.save(entity);
     }
     ...
     @Delete("/:idBadge")
     @UseBefore(AdminsAndTeachersOnly)
     delete(@Param("idBadge") idBadge: number) {             
