...
import { GroupsModel } from '../../../main.app/entities/classroom/GroupsModel';
 ...
 export class ApiGroupsController {
     ...
     @Get("/list")
     list(@QueryParam("idCourse", {required: true}) idCourse: number, @QueryParam("idCreator") idCreator: number) {  
     ...
     save(@Body({validate: true, required: true}) entity: GroupsModel) {        
         return this.groupSrv.save(entity);
     }
 
     @Put("/:id")
     @UseBefore(AdminsAndTeachersOnly)
     update(@Param("id") id: number, @Body({validate: true, required: true}) entity: GroupsModel) { 
         entity.id = id;       
         return this.groupSrv.save(entity);
     }
     ...
     @Delete("/:idGroup")
     @UseBefore(AdminsAndTeachersOnly)
     async delete(@Param("idGroup") idGroup: number) {     
