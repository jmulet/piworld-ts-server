...
export class AttemptModel {
    ...
    @ManyToOne((type)=>(SectionAssignModel), (sectionAssign)=>sectionAssign._attempts)
    @JoinColumn({name: "idSectionAssign"})
    _sectionAssign: SectionAssignModel;
    
    @OneToMany((type)=>(QuestionModel), (question)=>question._attempt, {onDelete: "CASCADE", cascade: ["remove"]})
