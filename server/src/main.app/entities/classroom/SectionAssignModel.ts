...
export class SectionAssignModel {
    ...
    @ManyToOne( (type)=> GroupsModel, (group) => group._sectionAssignments, {onDelete: "CASCADE"})
    @JoinColumn({name: "idGroup"})
    _group: GroupsModel;

    @OneToMany((type)=>(AttemptModel), (attempt)=>attempt._sectionAssign, {onDelete: "CASCADE"})
